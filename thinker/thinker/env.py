...
class PostVecModelWrapper(gym.Wrapper):
    ...
    def initial(self, model_net=None):
        ...
        if self.model_wrap:
            ...
            gym_env_out = torch.tensor(self.env.reset(model_net), dtype=torch.uint8, device=self.device)
            model_out = None
            model_encodes = None
        ...
        ret = EnvOut(
            ...
            max_rollout_depth=torch.zeros(1, self.env_n, dtype=torch.long, device=self.device) if self.model_wrap else None
        )
        return ret
    ...
    def step(self, action, model_net=None):
        ...
        if self.model_wrap:
            ...
            if np.any(done):
                ...
                gym_env_out[done] = torch.tensor(out_, dtype=torch.uint8, device=self.device)
        ...
        ret = EnvOut(
            ...
            last_action=self.last_action.unsqueeze(0),
            max_rollout_depth=info["max_rollout_depth"].unsqueeze(0) if self.model_wrap else None
        )
