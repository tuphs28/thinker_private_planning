...
class ActorBuffer():
    ...
    def read(self):
        ...
        if not self.available_for_read(): return None
        ...
        output = n_tuple(*(np.concatenate([data[i] for data in collected_data], axis=1)
            if collected_data[0][i] is not None else None for i in range(len(collected_data[0]))))
        output_state = tuple(np.concatenate([state[i] for state in collected_state], axis=1) for i in range(len(collected_state[0])))
        ...
        if collected_size > self.batch_size:
            extra_data = n_tuple(*(data[:, -collected_size + self.batch_size:, ...] if data is not None else None
                                   for data in output))
            self.buffer.insert(0, extra_data)
            extra_state = tuple(state[:, -collected_size + self.batch_size:, ...] for state in output_state)            
            self.buffer_state.insert(0, extra_state)

            # Trim the output to have the exact batch size
            output = n_tuple(*(data[:, :self.batch_size, ...] if data is not None else None 
                               for data in output))
            output_state = tuple(state[:, :self.batch_size, ...] for state in output_state)
...
class ModelBuffer():
    def __init__(self, flags):
        ...
        self.next_inds = None
        self.cur_inds = [None for _ in range(flags.gpu_num_actors+flags.cpu_num_actors)]
    ...
    def write(self, data, rank):
        ...
        if self.cur_inds[rank] is not None:
            ...
            mask = last_ind >= 0
            self.next_inds[last_ind[mask]] = (len(self.next_inds) + self.base_ind + np.arange(n))[mask]

        if self.next_inds is None:
            self.next_inds = np.full((n), fill_value=np.nan,)
        else:
            self.next_inds = np.concatenate([self.next_inds, np.full((n), fill_value=np.nan)])
    ...
    def prepare(self, beta):
        ...
        for d in range(len(self.buffer[0])):
            ...
            for i in range(self.batch_size):
                elems.append(self.buffer[inds[0][i]][d][inds[1][i]:inds[1][i]+2*self.k, np.newaxis])                
    ...
    def update_priority(self, abs_flat_inds, priorities):
        ...
        carry_inds_block = self.next_inds[flat_inds_block[carry_mask]-1] - self.base_ind   # the correct index block

        flat_inds = flat_inds.astype(float)
        flat_inds[carry_mask] = flat_inds[carry_mask] + (-flat_inds_block[carry_mask] + carry_inds_block) * self.t  
