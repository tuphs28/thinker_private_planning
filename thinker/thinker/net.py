...
class Output_rvpi(nn.Module):
    def __init__(
        ...
        if predict_r and self.value_prefix:
            self.conv1x1_reward = nn.Conv2d(
                in_channels=c, out_channels=16, kernel_size=1
            )
    ...
    def forward(self, h, predict_reward=True, state={}):
        ...
        if self.predict_r and predict_reward:
            if self.value_prefix:
                ...
                if torch.any(~m):
                    lstm_state = (
                        state[self.prefix + "r_lstm_0"] * m.float().view(b, 1),
                        state[self.prefix + "r_lstm_1"] * m.float().view(b, 1),
                    )
                    lstm_counter = state[self.prefix + "r_lstm_c"] * m.float()
                    last_r = state[self.prefix + "r_last_r"] * m.float()
                else:
                    lstm_state = (
                        state[self.prefix + "r_lstm_0"],
                        state[self.prefix + "r_lstm_1"],
                    )
                ...
                lstm_state = (
                    lstm_state[0].unsqueeze(0),
                    lstm_state[1].unsqueeze(0),
                )  # the LSTM only has a single layer
                ...
                state_.update(
                    {
                        self.prefix + "r_lstm_0": lstm_state[0],
    ...
    def init_state(self, bsz, device):
        if self.predict_r and self.value_prefix:
            return {
                self.prefix + "r_lstm_0": torch.zeros(bsz, 512, device=device),
