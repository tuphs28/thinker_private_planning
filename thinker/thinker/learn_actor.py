...
class SActorLearner:
        ...
        if self.timer() - self.start_time > 5:
            ...
            print_str = (
                ...
                % (
                    ...
                    stats["rmean_cur_episode_return"],
                    total_loss,
                )
            )
            print_stats = [
    ...
    def compute_losses(self, train_actor_out, initial_actor_state):
        ...
        if self.flags.cur_cost > 0.0 and self.real_step > self.flags.cur_warm_up_n:
            discounts = (~train_actor_out.done).float() * self.im_discounting
            behavior_logits_ls = [
                train_actor_out.policy_logits,
                train_actor_out.im_policy_logits,
            ...
            masks_ls = [real_mask, im_mask, im_mask]
            c_ls = [
                self.flags.cur_cost * self.anneal_c
                if self.flags.cur_cost_anneal
                else self.flags.cur_cost,
                self.flags.cur_im_cost * self.anneal_c
                if self.flags.cur_cost_anneal
                else self.flags.cur_im_cost,
                self.flags.cur_im_cost * self.anneal_c
                if self.flags.cur_cost_anneal
                else self.flags.cur_im_cost,
            ]
            ...
            vtrace_returns = from_logits(
                ...
                values_enc=new_actor_out.baseline_enc[:, :, 2]
                if new_actor_out.baseline_enc is not None
                else None,
                rv_tran=self.actor_net.rv_tran,
                enc_type=self.flags.critic_enc_type,
            ...
            cur_pg_loss = compute_policy_gradient_loss(
                target_logits_ls, actions_ls, masks_ls, c_ls, advs
            )
            cur_baseline_loss = self.flags.baseline_cost * compute_baseline_loss(
                ...
                target_baseline=vtrace_returns.vs,
                actor_net=self.actor_net,
                c=self.flags.cur_cost
                if not self.flags.cur_cost_anneal
                else self.flags.cur_cost * self.anneal_c,
                flags=self.flags,
            )
        else:
        ...
        if self.flags.im_cost > 0.0:
            ...
            if not self.flags.im_cost_anneal:
                c_ls = [
                    self.flags.im_cost,
                    self.flags.im_cost if not self.flags.reset_no_im_cost else 0.0,
                ]
            else:
                c_ls = [
    ...
    def compute_stat(self, train_actor_out, losses, total_norm, actor_id):
        ...
        if self.flags.cur_cost > 0.0:
            self.last_cur_returns.insert_raw(
                train_actor_out.episode_return,
                ind=2,
                actor_id=actor_id,
        ...
        if self.flags.return_norm_type != -1:
            ...
            stats["im_norm_diff"] = 0.0
            stats["cur_norm_diff"] = 0.0
            if self.im_norm_stat is not None:
                stats["im_norm_diff"] = (
                    self.im_norm_stat[1] - self.im_norm_stat[0]
                ).item()
            if self.cur_norm_stat is not None:
                stats["cur_norm_diff"] = (
                    self.cur_norm_stat[1] - self.cur_norm_stat[0]
                ).item()
