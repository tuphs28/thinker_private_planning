...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=f"Test for Thinker v{util.__version__}")
    ...
    parser.add_argument("--greedy", action="store_true",
                        help="Whether to select greedy action.")     
    parser.add_argument("--test_eps_n", default=100, type=int, 
                        help="Number of episode to test for")         
    ...
    buffers = {
        ...
        "test": GeneralBuffer.options(num_cpus=1).remote(),
    }
    ...
    self_play_workers = [SelfPlayWorker.options(
            ...
            flags=flags,
            base_seed=flags.base_seed) for n in range(flags.num_actors)]
    
    r_worker = [x.gen_data.remote(test_eps_n=test_eps_n_per_actor) for x in self_play_workers]   
    ray.get(r_worker)

    all_returns = ray.get(buffers["test"].get_data.remote("episode_returns"))  
    all_returns = np.array(all_returns)    
    if flags.greedy: 
        prefix="greedy_"
    else:
        prefix=""
    save_np_array_to_csv(array=all_returns, path=flags.load_checkpoint, prefix=prefix, logger=logger)  
