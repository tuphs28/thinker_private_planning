...
@torch.no_grad()
def create_probing_data(drc_net: DRCNet, env: Env, flags: NamedTuple, num_episodes: int, current_board_feature_fncs: list, future_feature_fncs: list, binary_feature_fncs: list, prob_accept: float = 1.0, debug: bool = False, inc_bl: bool = False) -> list:
    ...
    if inc_bl:
        bl_actor_out, bl_rnn_state = bl_drc_net(env_out, bl_rnn_state, greedy=True)
    ...
    if inc_bl:
        trans_entry["bl_hidden_states"] = bl_drc_net.hidden_state[0].detach().cpu()
    ...
    while(board_num < num_episodes):
        ...
        trans_entry["board_num"] = board_num
        episode_length += 1
    print("pct solved:", (num_solved / num_episodes) * 100)
