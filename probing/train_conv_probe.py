...
if __name__ == "__main__":
    ...
    for feature in features:
        ...
        for layer_name, layer_idx in layers:
            ...
            for epoch in range(1, num_epochs+1):
                ...
                if epoch % 25 == 0:
                    with torch.no_grad():
                        ...
                        if probe_args["positive_feature"] is not None and out_dim == 2:
                            ...
                            print("F1: ", fone)
                        elif probe_args["positive_feature"] is not None and out_dim != 2:
                            for j in range(out_dim):
                                print(f"-- Out Dim {j} --")
                                precisions[j] = prop_pos_cors[j] / (len(val_dataset.data))
                                recalls[j] = positive_accs[j] / (len(val_dataset.data))
            ...
            if out_dim == 2:
                results[layer_name] = {"Acc": full_acc/(len(val_dataset.data)*64), "Precision": precision, "Recall": recall, "F1": fone, "weights": convprobe.conv.weight.view(2,len(channels)).tolist()}
            elif out_dim == 5:
